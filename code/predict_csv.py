...
def predict(file, rasio, epoch, batch_size, neuron):
    currency = "EURUSD"
    interval = "5m"
    len_data = "1000"
    ...
    history = model.fit(train_X, train_y, epochs=epoch, batch_size=batch_size, validation_data=(test_X, test_y), verbose=1,
                        shuffle=False)
    t2 = time()
    print("Waktu Training: ", round(t2-t1))
    ...
    pred = "%.5f" % inv_yhat_predict_new[0]
    # act = "%.5f" % df['close'][start+1]

    return time_predict_data, pred
