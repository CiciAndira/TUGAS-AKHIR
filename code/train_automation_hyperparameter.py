...
df = scrap_csv("dataset/EURUSD_M5_Desember.csv")
df = df[['time','close','open','high','low','ma5','rsi','macd_l','macd_s','tenkan_sen','kijun_sen']]

split_rasio = [0.6, 0.7, 0.8, 0.9]
    ...
    for batch_size_for in batch_size:
        for neuron_for in neuron:
            for splits in split_rasio:
                ...
                n_train_hours = round(len(df)*rasio)
                ...
                history = model.fit(train_X, train_y, epochs=epoch_for, batch_size=batch_size_for,
                ...
                row_contents = [epoch_for, batch_size_for, neuron_for, splits, rmse, mape_result]
