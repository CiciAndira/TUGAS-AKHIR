...
def scrap_realtime():
    ...
    # get 10 EURUSD H4 bars starting from 01.10.2020 in UTC time zone
    rates = mt5.copy_rates_from("EURUSD", mt5.TIMEFRAME_M5, time, 1060)
    # rates = mt5.copy_rates_from_pos("EURUSD", mt5.TIMEFRAME_M5, 0, 1060)
...
def scrap_csv(filename):
    df = pd.read_csv(filename, sep='\t')
    df.rename(columns={'<DATE>': 'date', '<TIME>': 'time', '<OPEN>': 'open', '<HIGH>': 'high', '<LOW>': 'low',
                       '<CLOSE>': 'close', '<TICKVOL>': 'tick_volume', '<SPREAD>': 'spread', '<VOL>': 'vol'},
              inplace=True)
    df['datetime'] = pd.to_datetime(df['date'] + ' ' + df['time'])
    df = df.drop(columns=['date', 'time'])
    df = df[['datetime', 'open', 'high', 'low', 'close', 'tick_volume', 'spread', 'vol']]

    rates_frame = df
    rates_frame['datetime'] = pd.to_datetime(rates_frame['datetime'], unit='s')
    rates_frame["open"] = pd.to_numeric(rates_frame["open"], downcast="float")
    rates_frame["high"] = pd.to_numeric(rates_frame["high"], downcast="float")
    rates_frame["low"] = pd.to_numeric(rates_frame["low"], downcast="float")
    rates_frame["close"] = pd.to_numeric(rates_frame["close"], downcast="float")
    rates_frame["tick_volume"] = pd.to_numeric(rates_frame["tick_volume"], downcast="float")
    rates_frame = rates_frame.rename(columns={'datetime': 'time'})
